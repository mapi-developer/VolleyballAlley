...
export default function EventDetailsSheet({ isOpen, onClose, game, onRsvp, onCancelRsvp, isCancelling = false }: EventDetailsSheetProps) {
  ...
  const isCancellable = (new Date(game.rawDate).getTime() - Date.now()) > (2 * 60 * 60 * 1000);
        ...
        <div className="space-y-3">
          ...
          {game.isHost ? (
            <button disabled className="w-full py-4 rounded-2xl font-black text-base shadow-none bg-zinc-100 text-gray-400 cursor-not-allowed">
             ...
             isCancellable ? (
                <button onClick={() => onCancelRsvp?.(game.id)} disabled={isCancelling} className="w-full py-4 rounded-2xl font-black text-base shadow-xl active:scale-95 transition-all bg-rose-50 text-rose-600 shadow-rose-100 flex justify-center items-center">
