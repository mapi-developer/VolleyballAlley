...
export default function CreateEventSheet({ isOpen, onClose, onCreate }: CreateEventSheetProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const eventDateTime = new Date(`${formData.date}T${formData.startTime}:00`).getTime();
    if (eventDateTime < Date.now()) {
    ...
    const newEvent = {
      ...
      attendees: [{ id: 'host', name: user?.first_name || 'You', role: 'Organizer' }], 
      waitlist: []
    };
