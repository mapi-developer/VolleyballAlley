...
export default function CreateEventSheet({ isOpen, onClose, onCreate }: CreateEventSheetProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (startDateTime.getTime() < Date.now()) return alert("You cannot create an event in the past!");
    ...
    <BottomSheet isOpen={isOpen} onClose={onClose} title="Create New Event">
      <form onSubmit={handleSubmit} className="space-y-6 pb-4">
        ...
        <FormField label="Description" icon={AlignLeft}>
          <textarea rows={2} placeholder="Tell players about the game style..." className="w-full bg-zinc-50 border-none rounded-2xl p-4 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none resize-none" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
        </FormField>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Game Type" icon={MapIcon}>
            <select className="w-full bg-zinc-50 border-none rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none appearance-none" value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Max Players" icon={Users}>
            <input type="number" min="2" className="w-full bg-zinc-50 border-none rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" value={formData.maxPlayers} onChange={e => setFormData({ ...formData, maxPlayers: e.target.value })} />
          </FormField>
        </div>
        ...
        <div className="pt-4 border-t border-gray-100 flex flex-col gap-4">
          ...
          <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-base shadow-xl shadow-blue-100 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
